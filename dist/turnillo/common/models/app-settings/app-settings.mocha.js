...
const app_settings_fixtures_1 = require("./app-settings.fixtures");
describe("AppSettings", () => {
    var context = app_settings_fixtures_1.AppSettingsFixtures.getContext();
    it("is an immutable class", () => {
        immutable_class_tester_1.testImmutableClass(app_settings_1.AppSettings, [
            app_settings_fixtures_1.AppSettingsFixtures.wikiOnlyJS(),
            app_settings_fixtures_1.AppSettingsFixtures.wikiTwitterJS()
        ], { context });
    });
    describe("errors", () => {
        it("errors if there is no matching cluster", () => {
            var js = app_settings_fixtures_1.AppSettingsFixtures.wikiOnlyJS();
            js.clusters = [];
            chai_1.expect(() => app_settings_1.AppSettings.fromJS(js, context)).to.throw("Can not find cluster 'druid-wiki' for data cube 'wiki'");
        });
    });
    describe.skip("back compat", () => {
        it("works with dataSources", () => {
            var oldJS = app_settings_fixtures_1.AppSettingsFixtures.wikiOnlyJS();
            oldJS.dataSources = oldJS.dataCubes;
            delete oldJS.dataCubes;
            chai_1.expect(app_settings_1.AppSettings.fromJS(oldJS, context).toJS()).to.deep.equal(app_settings_fixtures_1.AppSettingsFixtures.wikiOnlyJS());
        });
        it("deals with old config style", () => {
            var wikiDataCubeJS = data_cube_fixtures_1.DataCubeFixtures.WIKI_JS;
        ...
        it("deals with old config style no sourceListScan=disabled", () => {
            var oldJS = {
                ...
                dataSources: [
                    Object.assign(Object.assign({}, data_cube_fixtures_1.DataCubeFixtures.WIKI_JS), { clusterName: "druid" })
                ]
            };
            chai_1.expect(app_settings_1.AppSettings.fromJS(oldJS, context).toJS().clusters).to.deep.equal([
                {
    ...
    describe("general", () => {
        ...
        it("converts to client settings", () => {
            const settings = app_settings_fixtures_1.AppSettingsFixtures.wikiOnlyWithExecutor();
...
//# sourceMappingURL=app-settings.mocha.js.map